...
class CommonController
{
    ...
    public function verifyAuthorization($token, $boutique_id): bool
    {
        ...
        try {
            $request = "SELECT id FROM autorization WHERE user_id = ? AND boutique_id = ?";
            $r = $this->pdo->prepare($request);
            $r->execute([(int) $this->user_id, (int) $boutique_id]);
    ...
    protected function decodeToken($token)
    {
        ...
        $this->user_id = (int) $this->decodedJWT->other->id;
    }
     ...
     * @return  self
     */
    public function setResponse(bool $response, string $message, int $codeHttp , string $token = null , string $refreshToken = null , array $user = null)
    {
