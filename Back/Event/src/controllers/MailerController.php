...
class MailerController extends CommonController
{
    ...
    public function sendMailModif($objet , $message , $email ){

        try {
            ob_start();
            include DIR."/src/templateMail/modifPlanning.php";
            $messageMail = ob_get_clean();
            $this->mailphp($objet , $messageMail , [$email]);
        } catch (\Throwable $th) {
    ...
    public function mailphp($subject, $message, array $to = ["contact@mavillemaboutique.fr"] )
    {
        ...
        try {
            ...
            $this->logError(date("d-m-Y H:i:s " ) . $e , "mail");
            return false;
        }
    }
}
